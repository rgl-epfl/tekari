...
public:
    ...
    Vector3f        average_point        (unsigned int wave_length_index) const { return m_average_point.col(wave_length_index); }
...
private:
    void add_point(const VectorXf& raw_point, const MatrixXf& transformed_point);

    void normalize_average();
    void normalize();

    unsigned int m_points_count;
    MatrixXf m_average_point;
